...
for DS in REDDIT-BINARY REDDIT-MULTI-5K; do
   ...
   python -m benchmark.cv $ARGS --dataset=$DS --q=0.8 --to_pickle=results/${POOL}_Q0.80_${DS}.pickle --batch_size=2000
   python -m benchmark.cv $ARGS --dataset=$DS --simplify --to_pickle=results/${POOL}_Simplify_${DS}.pickle --batch_size=20 --dense --dense_from=1
done
...
for DS in IMDB-BINARY IMDB-MULTI COLLAB; do
    ...
    python -m benchmark.cv $ARGS $OPT --dataset=$DS --q=0.8 --to_pickle=results/${POOL}_Q0.80_${DS}.pickle
    python -m benchmark.cv $ARGS $OPT --dataset=$DS --simplify --to_pickle=results/${POOL}_Simplify_${DS}.pickle
done