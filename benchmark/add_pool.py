...
def _add_pool_x(cluster, x, size=None):
    return scatter_('add', x, cluster, dim=0, dim_size=size)


def add_pool_x(cluster, x, batch, size=None):
    r"""Add pools node features according to the clustering defined in
    :attr:`cluster`.
    ...
    """
    if size is not None:
        return _add_pool_x(cluster, x, (batch.max().item() + 1) * size)

    cluster, perm = consecutive_cluster(cluster)
    x = _add_pool_x(cluster, x)
    batch = pool_batch(perm, batch)

    return x, batch


def add_pool(cluster, data, transform=None):
    ...
    Final node features are defined by the *sum* of the features of all nodes
